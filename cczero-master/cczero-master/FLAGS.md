...
`cc0` supports several operating modes, each on which has different set of 
command line flags (although there are common ones).

Currently `cc0` has the following modes:
...
To run `cc0` in any of those modes, specify a mode name as a first argument (`uci` may be omitted).
For example:

```bash
$ ./cc0 selfplay ...    # for selfplay mode
$ ./cc0 ...             # for uci mode
```
...
```bash
$ ./cc0 --help            # help for uci mode
$ ./cc0 selfplay --help   # help for selfplay mode
```
