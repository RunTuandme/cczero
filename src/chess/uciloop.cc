...
void UciLoop::SendResponses(const std::vector<std::string>& responses) {
    ...
    for (auto& response : responses) {
        if (debug_log_)
            debug_log_ << '<' << response << std::endl << std::flush;
        std::cout << response << std::endl;
    }
}

// TODO: remove these monstrosities
#define CC0V_STR_INNER(a) #a
#define CC0V_STR(a) CC0V_STR_INNER(a)
#define CC0_VERSION_STRING                        \
    "v" CC0V_STR(CC0_VERSION_MAJOR) "." CC0V_STR( \
        CC0_VERSION_MINOR) "." CC0V_STR(CC0_VERSION_PATCH)

void UciLoop::SendId() {
    SendResponse("id name The CC0 chess engine. " CC0_VERSION_STRING);
    SendResponse("id author The CCZero Authors.");
}
#undef CC0V_STR_INNER
#undef CC0V_STR
#undef CC0_VERSION_STRING
