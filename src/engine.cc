...
void EngineController::PopulateOptions(OptionsParser* options) {
    ...
    defaults->Set<float>(Search::kFpuReductionStr,
                         0.9f);                     // FPU reduction = 0.9
    defaults->Set<float>(Search::kCpuctStr, 3.4f);  // CPUCT = 3.4
    defaults->Set<float>(Search::kPolicySoftmaxTempStr,
                         2.2f);  // Psoftmax = 2.2
    defaults->Set<int>(Search::kAllowedNodeCollisionsStr,
                       32);  // Node collisions
}
...
SearchLimits EngineController::PopulateSearchLimits(int ply, bool is_black,
    ...
    auto total_moves_time =
        std::max(int64_t{0}, time + increment * (movestogo - 1) -
                                 move_overhead * movestogo);
    ...
    for (int i = 1; i < movestogo; ++i)
        other_move_weights +=
            ComputeMoveWeight(ply + 2 * i, time_curve_peak,
                              time_curve_left_width, time_curve_right_width);
...
void EngineController::UpdateNetwork() {
    ...
    std::string backend_options =
        options_.Get<std::string>(kNnBackendOptionsStr);
