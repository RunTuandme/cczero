...
void Search::SendUciInfo() REQUIRES(nodes_mutex_) {
    ...
    uci_info_.score =
        290.680623072 * tan(1.548090806 * best_move_edge_.GetQ(0));
    uci_info_.pv.clear();
...
void Search::SendMovesStats() const {
    ...
    std::sort(edges.begin(), edges.end(),
              [&parent_q, &U_coeff](EdgeAndNode a, EdgeAndNode b) {
                  return std::forward_as_tuple(
                             a.GetN(), a.GetQ(parent_q) + a.GetU(U_coeff)) <
                         std::forward_as_tuple(
                             b.GetN(), b.GetQ(parent_q) + b.GetU(U_coeff));
              });
    ...
    for (const auto& edge : edges) {
        ...
        oss << "(P: " << std::setw(5) << std::setprecision(2)
            << edge.GetP() * 100 << "%) ";
...
void Search::UpdateRemainingMoves() {
    ...
    if (limits_.time_ms >= 0) {
        ...
        if (time_since_start > kSmartPruningToleranceMs) {
            auto nps =
                (1000LL * total_playouts_ + kSmartPruningToleranceNodes) /
            ...
            // Don't assign directly to remaining_playouts_ as overflow is
            // possible.
    ...
    if (limits_.visits >= 0) {
        ...
        // number.
        auto remaining_visits = limits_.visits - total_playouts_ -
                                initial_visits_ + kMiniBatchSize - 1;
...
void SearchWorker::GatherMinibatch() {
    ...
    while (nodes_found < search_->kMiniBatchSize) {
        ...
        // If node is already known as terminal (win/loss/draw according to
        // rules of the game), it means that we already visited this node
        // before.
...
SearchWorker::NodeToProcess SearchWorker::PickNodeToExtend() {
    ...
    while (true) {
        ...
        // Set 'node' to point to the node that was picked on previous
        // iteration, possibly spawning it.
        // TODO(crem) This statement has to be in the end of the loop rather
        // than
        ...
        // n_in_flight_ is incremented. If the method returns false, then there
        // is a search collision, and this node is already being expanded.
        ...
        float puct_mult = search_->kCpuct *
        ...
        float parent_q =
            ((is_root_node && search_->kNoise) || !search_->kFpuReduction)
                ? -node->GetQ()
                : -node->GetQ() - search_->kFpuReduction *
                                      std::sqrt(node->GetVisitedPolicy());
        for (auto child : node->Edges()) {
            if (is_root_node) {
                // If there's no chance to catch up to the current best node
                // with remaining playouts, don't consider it. best_move_node_
                // could have changed since best_node_n was retrieved. To ensure
                // we have at least one node to expand, always include current
                // best node.
                ...
                // If searchmoves was sent, restrict the search only in that
                // moves
...
bool SearchWorker::AddNodeToComputation(Node* node, bool add_if_cached) {
    ...
    if (node && node->HasChildren()) {
        ...
        for (auto iter = pseudolegal_moves.begin(),
                  end = pseudolegal_moves.end();
             iter != end; ++iter) {
...
void SearchWorker::MaybePrefetchIntoCache() {
    ...
    if (computation_->GetCacheMisses() > 0 &&
        ...
        PrefetchIntoCache(
            search_->root_node_,
            search_->kMaxPrefetchBatch - computation_->GetCacheMisses());
    }
}
...
int SearchWorker::PrefetchIntoCache(Node* node, int budget) {
    ...
    if (!node || node->GetNStarted() == 0) {
        if (AddNodeToComputation(node, false)) {
            // Make it return 0 to make it not use the slot, so that the
            // function tries hard to find something to cache even among
            // unpopular moves. In practice that slows things down a lot though,
            // as it's not always easy to find what to cache.
            return 1;
        }
        return 1;
    }
    ...
    for (size_t i = 0; i < scores.size(); ++i) {
        ...
        if (i != scores.size() - 1) {
            ...
            if (next_score > q) {
                budget_to_spend = std::min(
                    budget, int(edge.GetP() * puct_mult / (next_score - q) -
                                edge.GetNStarted()) +
                                1);
            } else {
        ...
        const int budget_spent =
            PrefetchIntoCache(edge.node(), budget_to_spend);
        history_.Pop();
        budget -= budget_spent;
...
void SearchWorker::FetchMinibatchResults() {
    ...
    for (auto& node_to_process : nodes_to_process_) {
        ...
        if (!node_to_process.nn_queried) {
            // Terminal nodes don't involve the neural NetworkComputation, nor
            // do they require any further processing after value retrieval.
