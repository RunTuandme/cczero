...
SelfPlayTournament::SelfPlayTournament(const OptionsDict& options,
    ...
    for (int idx : {0, 1}) {
        ...
        if (idx == 1) {
            ...
            for (const auto& option_str :
                ...
                if (options.GetSubdict("player1").Get<std::string>(
                        option_str) !=
                    options.GetSubdict("player2").Get<std::string>(
                        option_str)) {
    ...
    for (int idx : {0, 1}) {
        ...
        if (search_limits_[idx].playouts == -1 &&
            ...
            search_limits_[idx].time_ms == -1) {
            throw Exception(
                "Please define --visits, --playouts or --movetime, otherwise "
                "it's "
