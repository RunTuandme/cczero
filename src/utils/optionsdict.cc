   ...
   private:
    void ReadString() {
        ...
        for (; idx_ < str_.size(); ++idx_) {
            if (str_[idx_] == quote) {
                type_ = L_STRING;
                string_val_ =
                    str_.substr(last_offset_ + 1, idx_ - last_offset_ - 1);
                ++idx_;
                return;
            }
        }
    ...
    void ReadNumber() {
        ...
        for (; idx_ < str_.size(); ++idx_) {
            ...
            if (kFloatChars.find(str_[idx_]) != std::string::npos)
                is_float = true;
        }
        ...
        try {
            if (is_float) {
                type_ = L_FLOAT;
                float_val_ =
                    stof(str_.substr(last_offset_, idx_ - last_offset_));
...
class Parser {
   ...
   private:
    ...
    void ParseList(OptionsDict* dict) {
        while (true) {
            ...
            if (lexer_.GetToken() == Lexer::L_LEFT_PARENTHESIS) {
                // List entry starts with "(", that's a special case of subdict
                // without name, we have to come up with the name ourselves.
            ...
            // If there is "=" after identifier, that's key=value entry, read
            // value.
    ...
    void ReadSubDict(OptionsDict* dict, const std::string& identifier) {
        ...
        // If opening parentheses, read list of a subdict, otherwise list is
        // empty, so return immediately.
        if (lexer_.GetToken() == Lexer::L_LEFT_PARENTHESIS) {
            lexer_.Next();
            ParseList(new_dict);
